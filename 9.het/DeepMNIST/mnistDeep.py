...
def readimg():
    ...
    #kell az 1-es, hogy az output greyscale image legyen
    return img
...
def main(_):
  ...
  with tf.Session() as sess:
    ...
    print("-- A 42-es tesztkepet a halozat ennek ismeri fel: ", classification[0])
    print("----------------------------------------------------------")
    ...
    classification = sess.run(tf.argmax(y_conv, 1), feed_dict={x: [image], keep_prob: 1.0})
    #classification = sess.run(accuracy, feed_dict={x: [image], y_: mnist.test.labels, keep_prob:1.0})

    print("-- A sajat szamunkat a halozat ennek ismeri fel: ", classification[0])
    print("----------------------------------------------------------")
