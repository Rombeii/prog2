...
public:
  Perceptron ( int nof, ... )
  {
    ...
    count += sizeof(double*)*n_layers;//memóriafoglalás
    count += sizeof(int)*n_layers;//memóriafoglalás
    ...
    for ( int i {0}; i < n_layers; ++i )
      {
        ...
        if ( i )
          ...
          count + sizeof(double)*n_units[i];//memóriafoglalás
      }

    va_end ( vap );

    weights = new double**[n_layers-1];
    count += sizeof(double**)*(n_layers-1);//memóriafoglalás
        ...
        for ( int j {0}; j < n_units[i]; ++j )
          {
            ...
            count += sizeof(double)*(n_units[i-1]);//memóriafoglalás
  ...
  double operator() ( double image [] )
  {
    ...
    //hozzáírva, a 8. tétel miatt kell
  ...
  {
    //( *this ) ( image );//EZ ALAPBA KI VOLT KOMMENTELVE
